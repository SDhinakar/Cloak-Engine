...
program
  .name("cloakscan")
  .description(
    "CloakScan - Privacy-first AI Code Review CLI with comprehensive security scanning"
  )
...
program
  .command("init")
  .description(
    "Initialize CloakScan (optional - generates client_id for telemetry)"
  )
  .action(initCommand);
...
if (globalOpts.noTelemetry) {
  ...
  process.env.CLOAKSCAN_NO_TELEMETRY = "true";
}
